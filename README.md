...
検索には出さない（`robots.txt` と `noindex`）。**ただしこれはクローラーへのお願いで、強制ではない。**

🔴 **いまの公開範囲＝リンクを知っていれば誰でも見られる**（2026-09-11 宮崎さん判断）。
さらに**このリポジトリが public なので、GitHubのアカウントからURLに辿り着ける**
（`raw.githubusercontent.com` から画像も直接落ちる）。
塞ぐならリポジトリを private にする（Pagesのサイトは公開のままでURLも変わらないが、
private リポでPagesを使うには有料プランが必要）。
